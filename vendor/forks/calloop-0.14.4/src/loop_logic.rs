...
impl<'l, Data> LoopHandle<'l, Data> {
        ...
        S: EventSource + 'l,
    {
        ...
        trace!("Inserting new source source={}", slot.token.get_id());
    ...
    pub fn enable(&self, token: &RegistrationToken) -> crate::Result<()> {
        ...
        {
            trace!("Registering source source={}", entry_token.get_id());
            source.register(
    ...
    pub fn update(&self, token: &RegistrationToken) -> crate::Result<()> {
        ...
        {
            trace!(
                "Updating registration of source source={}",
                entry_token.get_id()
            );
            if !source.reregister(
                ...
                trace!(
                    "Can't update registration withing a callback, storing for later source={}",
                    entry_token.get_id()
                );
    ...
    pub fn disable(&self, token: &RegistrationToken) -> crate::Result<()> {
        ...
        } = self.inner.sources.borrow().get(token.inner)?
        {
            ...
            trace!("Unregistering source source={}", entry_token.get_id());
            if !source.unregister(
                ...
                *token,
                ...
                trace!(
                    "Cannot unregister source in callback, storing for later source={}",
                    entry_token.get_id()
                );
                // we are in a callback, store for later processing
    ...
    pub fn remove(&self, token: RegistrationToken) {
        ...
        }) = self.inner.sources.borrow_mut().get_mut(token.inner)
        {
            if let Some(source) = source.take() {
                trace!("Removing source source={}", entry_token.get_id());
    ...
    fn dispatch_events(
        ...
        for event in self.synthetic_events.drain(..).chain(events) {
            ...
            if let Some(disp) = opt_disp {
                trace!("Dispatching events for source source={}", reg_token.get_id());
                ...
                match ret {
                    PostAction::Reregister => {
                        trace!(
                            "Postaction reregister for source source={}",
                            reg_token.get_id()
                        );
                        disp.reregister(
                    ...
                    PostAction::Disable => {
                        trace!(
                            "Postaction unregister for source source={}",
                            reg_token.get_id()
                        );
                        disp.unregister(
                            &mut self.handle.inner.poll.borrow_mut(),
                    ...
                    PostAction::Remove => {
                        trace!(
                            "Postaction remove for source source={}",
                            reg_token.get_id()
                        );
                ...
                warn!("Received an event for non-existent source: {reg_token:?}");
            }
        }
