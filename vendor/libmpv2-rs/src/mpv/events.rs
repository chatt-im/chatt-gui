...
impl Mpv {
    /// Request log messages at `min_level` or above on this client handle.
    ///
    /// Common levels are `fatal`, `error`, `warn`, `info`, `v`, `debug`,
    /// `trace`, and `no`. Requested messages are delivered as
    /// [`Event::LogMessage`].
    pub fn request_log_messages(&self, min_level: &str) -> Result<()> {
        let min_level = CString::new(min_level)?;
        mpv_err((), unsafe {
            libmpv2_sys::mpv_request_log_messages(self.ctx.as_ptr(), min_level.as_ptr())
        })
    }
