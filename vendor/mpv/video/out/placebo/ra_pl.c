...
struct ra_pl {
    ...
    pl_buf *upload_buffers;
    int num_upload_buffers;
    int next_upload_buffer;
};

#define MAX_UPLOAD_BUFFERS 12
...
{
    struct ra_pl *p = ra->priv;
    for (int i = 0; i < p->num_upload_buffers; i++)
        pl_buf_destroy(p->gpu, &p->upload_buffers[i]);
    talloc_free(ra);
}
...
static pl_buf get_upload_buffer(struct ra *ra, size_t size)
{
    struct ra_pl *p = ra->priv;

    for (int n = 0; n < p->num_upload_buffers; n++) {
        int index = (p->next_upload_buffer + n) % p->num_upload_buffers;
        pl_buf buffer = p->upload_buffers[index];
        if (buffer->params.size >= size && !pl_buf_poll(p->gpu, buffer, 0)) {
            p->next_upload_buffer = (index + 1) % p->num_upload_buffers;
            return buffer;
        }
    }

    if (p->num_upload_buffers < MAX_UPLOAD_BUFFERS) {
        pl_buf buffer = pl_buf_create(p->gpu, pl_buf_params(
            .size = size,
            .host_writable = true,
        ...
        if (!buffer)
            return NULL;
        MP_TARRAY_APPEND(p, p->upload_buffers, p->num_upload_buffers, buffer);
        p->next_upload_buffer = p->num_upload_buffers % MAX_UPLOAD_BUFFERS;
        MP_VERBOSE(ra, "Created host upload buffer %d with %zu bytes.\n",
                   p->num_upload_buffers, size);
        return buffer;
    }

    int index = p->next_upload_buffer;
    pl_buf buffer = p->upload_buffers[index];
    while (pl_buf_poll(p->gpu, buffer, UINT64_MAX)) {}
    if (buffer->params.size < size) {
        if (!pl_buf_recreate(p->gpu, &buffer, pl_buf_params(
                .size = size,
                .host_writable = true,
                .memory_type = PL_BUF_MEM_HOST,
            )))
            return NULL;
        p->upload_buffers[index] = buffer;
    }
    p->next_upload_buffer = (index + 1) % p->num_upload_buffers;
    return buffer;
}
...
static bool tex_upload_pl(struct ra *ra, const struct ra_tex_upload_params *params)
{
    ...
    if (params->tex->params.dimensions == 2) {
        ...
        int width = params->rc ? mp_rect_w(*params->rc) : tex->params.w;
        int height = params->rc ? mp_rect_h(*params->rc) : tex->params.h;
        size_t row_pitch = params->stride;
        size_t row_bytes = width * tex->params.format->texel_size;
        if (params->src && width > 0 && height > 0 && row_pitch >= row_bytes &&
            (size_t)(height - 1) <= (SIZE_MAX - row_bytes) / row_pitch)
        {
            size_t size = (height - 1) * row_pitch + row_bytes;
            pl_buf staging = get_upload_buffer(ra, size);
            if (!staging)
                return false;
            pl_buf_write(gpu, staging, 0, params->src, size);
    ...
    return pl_tex_upload(gpu, &pl_params);
}
