...
{
    struct mpvk_ctx *vk = ctx->user_opaque;
    if (vk->lock_queue)
        vk->lock_queue(vk->queue_ctx, queue_family, index);
    else
        vk->vulkan->lock_queue(vk->vulkan, queue_family, index);
}
                         ...
                         uint32_t queue_family, uint32_t index)
{
    struct mpvk_ctx *vk = ctx->user_opaque;
    if (vk->unlock_queue)
        vk->unlock_queue(vk->queue_ctx, queue_family, index);
    else
        vk->vulkan->unlock_queue(vk->vulkan, queue_family, index);
}

#if LIBAVUTIL_VERSION_INT < AV_VERSION_INT(59, 34, 100)
static int enabled_queue_count(struct mpvk_ctx *vk, uint32_t family,
                               uint32_t physical_count)
{
    if (vk->vkinst)
        return physical_count;

    for (int i = 0; i < vk->num_enabled_queue_families; i++) {
        if (vk->enabled_queue_families[i].index == family)
            return vk->enabled_queue_families[i].count;
    }
    return 0;
}
#endif
...
static int vulkan_init(struct ra_hwdec *hw)
{
...
#endif
    device_hwctx->get_proc_addr = vk->vkinst ? vk->vkinst->get_proc_addr
                                             : vk->get_proc_addr;
    device_hwctx->inst = vk->vkinst ? vk->vkinst->instance
                                    : vk->vulkan->instance;
    ...
    device_hwctx->enabled_inst_extensions = vk->vkinst
        ? vk->vkinst->extensions
        : (const char *const *)vk->instance_extensions;
    ...
    if (!vk->vkinst) {
        for (int i = 0; i < vk->num_enabled_queue_families; i++) {
            uint32_t family = vk->enabled_queue_families[i].index;
            if (family >= num_qf || device_hwctx->nb_qf >= MP_ARRAY_SIZE(device_hwctx->qf))
                continue;
            device_hwctx->qf[device_hwctx->nb_qf++] = (AVVulkanDeviceQueueFamily) {
                .idx = family,
                .num = vk->enabled_queue_families[i].count,
                .flags = qf[family].queueFamilyProperties.queueFlags,
                .video_caps = qf_vid[family].videoCodecOperations,
            };
        }
    } else {
        device_hwctx->qf[device_hwctx->nb_qf++] = (AVVulkanDeviceQueueFamily) {
    ...
    for (int i = 0; i < num_qf; i++) {
        if ((qf[i].queueFamilyProperties.queueFlags) & VK_QUEUE_VIDEO_DECODE_BIT_KHR &&
            enabled_queue_count(vk, i, qf[i].queueFamilyProperties.queueCount))
            decode_index = i;
    }
    ...
    device_hwctx->nb_decode_queues = decode_index < 0 ? 0 : enabled_queue_count(
        vk, decode_index, qf[decode_index].queueFamilyProperties.queueCount);
#endif
