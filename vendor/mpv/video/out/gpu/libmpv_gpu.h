...
struct libmpv_gpu_context_fns {
    ...
    int (*wrap_fbo)(struct libmpv_gpu_context *ctx, mpv_render_param *params,
                    struct ra_tex **out);
    // Handle a backend-specific mpv_render_context_set_parameter() call.
    int (*set_parameter)(struct libmpv_gpu_context *ctx, mpv_render_param param);
    ...
    int (*done_frame)(struct libmpv_gpu_context *ctx, bool ds);
...
#if HAVE_VULKAN
extern const struct libmpv_gpu_context_fns libmpv_gpu_context_vk;
#endif
