...
impl WgpuRenderer {
    ...
    pub fn draw(&mut self, scene: &Scene) -> bool {
        ...
        // Path rendering needs a full-window intermediate texture and, on most
        // adapters, a multisampled companion. Keep those allocations lazy for
        // scenes composed only of quads, text, and sprites.
        ...
        loop {
            ...
            for surface in &scene.surfaces {
                if let Some(video) = crate::video_surface_from_platform(&surface.source) {
                    video.finish_draw();
                }
            }
            frame.present();
    ...
    fn draw_surfaces(
        ...
        for surface in surfaces {
            let Some(video) = crate::video_surface_from_platform(&surface.source) else {
                continue;
            };
            let Some(view) = video.prepare_draw() else {
                continue;
            };
            ...
            let buffer = resources.device.create_buffer(&wgpu::BufferDescriptor {
                label: Some("video_surface_params"),
