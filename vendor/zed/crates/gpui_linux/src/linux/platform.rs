...
#[cfg(feature = "xdg-desktop-portal")]
...
pub(crate) struct LinuxCommon {
    ...
    #[cfg_attr(not(feature = "xdg-desktop-portal"), allow(dead_code))]
    wake_sender: Sender<()>,
...
impl<P: LinuxClient + 'static> Platform for LinuxPlatform<P> {
    ...
    fn prompt_for_paths(
        ...
        #[cfg(not(feature = "xdg-desktop-portal"))]
                ...
                let result = match request.response() {
                    Ok(response) => Ok(Some(
                        response
                            .uris()
                            .iter()
                            .filter_map(|uri: &ashpd::Uri| file_uri_to_path(uri.as_str()))
                            .collect::<Vec<_>>(),
                    )),
                ...
                async move {
                    ...
                    let result = match request.response() {
                        Ok(response) => Ok(response
                            .uris()
                            .first()
                            .and_then(|uri: &ashpd::Uri| file_uri_to_path(uri.as_str()))),
                        Err(ashpd::Error::Response(_)) => Ok(None),
                        Err(e) => Err(e.into()),
                    };
                    let _ = done_tx.send(result);
    ...
    fn write_credentials(&self, url: &str, username: &str, password: &[u8]) -> Task<Result<()>> {
        ...
        #[cfg(not(feature = "credentials"))]
        {
            ...
            Task::ready(Err(anyhow!("credential storage is not enabled")))
        }
    }
    ...
    executor
        .spawn(async move {
            ...
            match command.status().await {
                ...
                Ok(status) => log::error!("xdg-open exited with status: {status}"),
                Err(error) => log::error!("failed to invoke xdg-open: {error}"),
            }
            ...
            if let Some(uri) = ashpd::Uri::parse(&uri).log_err() {
                    ...
                    .and_then(|response| response.response())
                {
                    ...
                    Err(error) => {
                        log::error!("Failed to open with dbus: {error}");
                    }
                }
            }
        })
    ...
    executor
        .spawn(async move {
            ...
            if let Err(error) = command.status().await {
                log::error!("failed to invoke xdg-open: {error}");
            }
        })
        .detach();
}
...
pub(super) fn file_uri_to_path(uri: &str) -> Option<PathBuf> {
    ...
    let encoded = if encoded.starts_with('/') {
        ...
        encoded.strip_prefix("localhost")?
    };
...
mod tests {
    ...
    #[test]
    fn decodes_local_file_uris_without_url_dependency() {
        assert_eq!(
            file_uri_to_path("file:///tmp/hello%20world.txt"),
            Some(PathBuf::from("/tmp/hello world.txt"))
        );
        assert_eq!(
            file_uri_to_path("file://localhost/tmp/file.txt"),
            Some(PathBuf::from("/tmp/file.txt"))
        );
    }

    #[test]
    fn rejects_remote_or_malformed_file_uris() {
        assert_eq!(file_uri_to_path("file://example.com/tmp/file.txt"), None);
