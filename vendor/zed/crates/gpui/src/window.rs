...
impl Window {
    ...
    /// Paint a persistent platform-renderer surface into the next frame.
    #[cfg(not(target_os = "macos"))]
    pub fn paint_platform_surface(
        &mut self,
        bounds: Bounds<Pixels>,
        source: crate::PlatformSurface,
    ) {
        use crate::PaintSurface;
        ...
        let bounds = self.snap_bounds(bounds);
        source.0.request_size(crate::size(
            DevicePixels(bounds.size.width.0.ceil() as i32),
            DevicePixels(bounds.size.height.0.ceil() as i32),
        ));
        let content_mask = self.snapped_content_mask();
        self.next_frame.scene.insert_primitive(PaintSurface {
            order: 0,
