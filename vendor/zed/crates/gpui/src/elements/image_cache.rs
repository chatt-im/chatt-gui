...
/// An image cache that retains every image until it is explicitly cleared or removed.
...
/// An image cache that evicts the least-recently-used decoded images to stay
/// within a byte budget.
///
/// `A` controls how resources are decoded. The default uses GPUI's standard
/// image loader, while callers can supply a loader that produces thumbnails or
/// applies other application-specific limits.
pub struct LruImageCache<A = ImageAssetLoader> {
    entries: HashMap<u64, LruImageCacheEntry>,
    budget_bytes: usize,
    total_bytes: usize,
    clock: u64,
    loader: PhantomData<fn() -> A>,
}

impl<A> fmt::Debug for LruImageCache<A> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    ...
    A: Asset<Source = Resource, Output = Result<Arc<RenderImage>, ImageCacheError>>,
{
    ...
    pub fn load(
        ...
        if let Some(entry) = self.entries.get_mut(&source_hash) {
            ...
            {
                entry.decoded_bytes = render_image_byte_len(image);
                self.total_bytes = self.total_bytes.saturating_add(entry.decoded_bytes);
            }
            if result.is_some() {
        ...
        let future = AssetLogger::<A>::load(source.clone(), cx);
    ...
    /// Clear every cached image and remove its texture from all windows.
    pub fn clear(&mut self, window: &mut Window, cx: &mut App) {
    ...
    /// Return the number of cached or loading resources.
    pub fn len(&self) -> usize {
        self.entries.len()
    }

    /// Return whether the cache contains no resources.
    pub fn is_empty(&self) -> bool {
        self.entries.is_empty()
    }

    /// Return the decoded bytes currently charged against the cache budget.
    pub fn total_bytes(&self) -> usize {
        self.total_bytes
    }

    fn evict(&mut self, protected_hash: u64, window: &mut Window, cx: &mut App) {
        while self.total_bytes > self.budget_bytes {
            ...
            let Some(mut entry) = self.entries.remove(&candidate) else {
                continue;
            };
            self.total_bytes = self.total_bytes.saturating_sub(entry.decoded_bytes);
            if let Some(Ok(image)) = entry.item.get() {
...
fn render_image_byte_len(image: &RenderImage) -> usize {
    (0..image.frame_count()).fold(0usize, |total, frame_index| {
