...
struct StateInner {
    ...
    #[allow(clippy::type_complexity)]
    visible_range_handler: Option<Box<dyn FnMut(Range<usize>, &mut Window, &mut App)>>,
...
impl ListState {
    ...
    /// Set a handler that will be called after layout whenever the range of
    /// items intersecting the viewport changes.
    ///
    /// Unlike [`Self::set_scroll_handler`], this also observes programmatic
    /// scrolling, resizes, and item remeasurement.
    pub fn set_visible_range_handler(
        &self,
        handler: impl FnMut(Range<usize>, &mut Window, &mut App) + 'static,
    ) {
        let state = &mut *self.0.borrow_mut();
        state.visible_range_handler = Some(Box::new(handler));
        state.last_visible_range = None;
    }
...
impl Element for List {
    ...
    fn prepaint(
        ...
        let item_count = state.items.summary().count;
        let visible_range = if bounds.size.height > padding.top + padding.bottom {
            let height = bounds.size.height - padding.top - padding.bottom;
            let range = StateInner::visible_range(&state.items, height, &layout.scroll_top);
            range.start.min(item_count)..range.end.min(item_count)
        } else {
            let start = layout.scroll_top.item_ix.min(item_count);
            start..start
        };
        if state.last_visible_range.as_ref() != Some(&visible_range) {
            state.last_visible_range = Some(visible_range.clone());
            if let Some(handler) = state.visible_range_handler.as_mut() {
