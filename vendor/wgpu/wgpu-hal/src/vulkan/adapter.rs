...
impl PhysicalDeviceFeatures {
    ...
    fn from_extensions_and_requested_features(
        ...
        Self {
            ...
            {
                Some(
                    vk::PhysicalDeviceHostQueryResetFeatures::default().host_query_reset(
                        phd_features
                            .host_query_reset
                            .as_ref()
                            .is_some_and(|features| features.host_query_reset == vk::TRUE),
                    ),
                )
            } else {
                None
...
impl PhysicalDeviceProperties {
    ...
    fn get_required_extensions(&self, requested_features: wgt::Features) -> Vec<&'static CStr> {
        ...
        // Allow Vulkan consumers sharing this device (notably libplacebo/mpv)
        // to import Linux dma-buf images with their DRM format modifiers. The
        // external-memory extensions above are not sufficient for tiled VA-API
        // decode surfaces, and omitting this extension forces mpv to download
        // every decoded frame to the CPU before uploading it again.
    ...
    fn inspect(
        ...
        features.core = if let Some(ref get_device_properties) = self.get_physical_device_properties
        {
            ...
            if capabilities.device_api_version >= vk::API_VERSION_1_2
                || capabilities.supports_extension(ext::host_query_reset::NAME)
            {
                let next = features
                    .host_query_reset
                    .insert(vk::PhysicalDeviceHostQueryResetFeatures::default());
                features2 = features2.push_next(next);
            }
...
impl super::Adapter {
    ...
    pub unsafe fn device_from_raw(
        ...
        let queue = super::Queue {
            ...
            family_index,
            host_lock: Arc::new(super::QueueHostLock::default()),
