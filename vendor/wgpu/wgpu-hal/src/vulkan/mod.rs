...
use parking_lot::{Mutex, RawMutex, RwLock, lock_api::RawMutex as _};
...
pub struct Queue {
    ...
    family_index: u32,
    host_lock: Arc<QueueHostLock>,
    relay_semaphores: Mutex<RelaySemaphores>,
    wait_semaphores: Mutex<SemaphoreList>,
    signal_semaphores: Mutex<SemaphoreList>,
}

/// A host-side lock shared by wgpu and users that submit work to the same
/// Vulkan queue through another API.
pub struct QueueHostLock {
    mutex: RawMutex,
}

impl Default for QueueHostLock {
    fn default() -> Self {
        Self {
            mutex: RawMutex::INIT,
        }
...
impl QueueHostLock {
    /// Acquire the queue lock. Each successful call must be paired with one
    /// [`unlock`](Self::unlock) call, normally from the same thread.
    pub fn lock(&self) {
        self.mutex.lock();
    }
    ...
    /// # Safety
    ///
    /// The current thread must hold this lock exactly once.
    pub unsafe fn unlock(&self) {
        unsafe { self.mutex.unlock() };
    }

    fn guard(&self) -> QueueHostLockGuard<'_> {
        self.lock();
        QueueHostLockGuard(self)
    }
}

struct QueueHostLockGuard<'a>(&'a QueueHostLock);

impl Drop for QueueHostLockGuard<'_> {
    fn drop(&mut self) {
...
impl Queue {
    ...
    /// Return the host lock guarding submissions and presentation on this
    /// queue. External Vulkan users must hold it around every queue operation.
    pub fn host_lock(&self) -> Arc<QueueHostLock> {
        Arc::clone(&self.host_lock)
    }
}
...
impl crate::Queue for Queue {
    ...
    unsafe fn submit(
        ...
        let mut external_waits = self.wait_semaphores.lock();
        if !external_waits.is_empty() {
            wait_semaphores.append(&mut external_waits);
        }
        ...
        let _host_guard = self.host_lock.guard();
        unsafe {
            self.device
                .raw
...
impl Queue {
    ...
    /// Add a binary or timeline semaphore wait to the next wgpu submission.
